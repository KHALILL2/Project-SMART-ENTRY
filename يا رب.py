    ...
    def control_green_led(self, state: bool) -> None:
        ...
        try:
            with self._lock:
                ...
                if state:
                    threading.Thread(target=self.buzz, args=(0.1, True)).start()
        except Exception as e:
            self._error_count += 1
    ...
    def control_red_led(self, state: bool) -> None:
        ...
        try:
            with self._lock:
                ...
                if state:
                    threading.Thread(target=self.buzz, args=(0.1, False)).start()
        except Exception as e:
            self._error_count += 1
            self.logger.log_error(e, f"Red LED control failed turning {action}")

    def buzz(self, duration: float = 0.1, is_valid: bool = True) -> None:
        if not self._is_initialized:
...
class AccessControlGUI:
    ...
    def _test_buzzer(self):
        ...
        self.gui_queue.put("Testing valid sound with green LED...")
        self.hardware.control_green_led(True)
        time.sleep(2)
        self.hardware.control_green_led(False)
        time.sleep(0.5)
        self.gui_queue.put("Testing warning sound with red LED...")
        self.hardware.control_red_led(True)
        time.sleep(2)
        self.hardware.control_red_led(False)
        self.gui_queue.put("Buzzer test completed.")
