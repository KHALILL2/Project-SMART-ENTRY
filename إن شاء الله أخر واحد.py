    ...
    class MockGPIO:
        ...
        class PWM:
            def __init__(self, pin, freq):
                ...
                self.freq = freq
                self.duty_cycle = 0
                print(f"MockGPIO: PWM initialized on pin {pin} with freq {freq}")
            def start(self, dc):
                self.duty_cycle = dc
                print(f"MockGPIO: PWM started on pin {self.pin} with duty cycle {dc}")
            def stop(self):
                self.duty_cycle = 0
                print(f"MockGPIO: PWM stopped on pin {self.pin}")
            def ChangeDutyCycle(self, dc):
                self.duty_cycle = dc
                print(f"MockGPIO: PWM duty cycle changed to {dc} on pin {self.pin}")
            def ChangeFrequency(self, freq):
                self.freq = freq
                print(f"MockGPIO: PWM frequency changed to {freq} on pin {self.pin}")
    GPIO = MockGPIO()
...
class HardwareController:
    ...
    def _initialize_hardware(self) -> None:
        with self._lock:
            ...
            try:
                ...
                self._buzzer_pwm = GPIO.PWM(self.config.BUZZER_PIN, 2000)
    ...
    def control_green_led(self, state: bool) -> None:
        if not self._is_initialized:
            self.logger.log_error(RuntimeError("Hardware not initialized, cannot control green LED."))
            return
        action = "ON" if state else "OFF"
        try:
            with self._lock:
                GPIO.output(self.config.GREEN_LED_PIN, GPIO.HIGH if state else GPIO.LOW)
                self.logger.log_info(f"Green LED turned {action}")
        except Exception as e:
            self._error_count += 1
            self.logger.log_error(e, f"Green LED control failed turning {action}")

    def control_red_led(self, state: bool) -> None:
        if not self._is_initialized:
            self.logger.log_error(RuntimeError("Hardware not initialized, cannot control red LED."))
            return
        action = "ON" if state else "OFF"
        try:
            with self._lock:
                GPIO.output(self.config.RED_LED_PIN, GPIO.HIGH if state else GPIO.LOW)
                self.logger.log_info(f"Red LED turned {action}")
        except Exception as e:
            self._error_count += 1
            self.logger.log_error(e, f"Red LED control failed turning {action}")

    def start_buzz(self, frequency=2000):
        """Start the buzzer with the specified frequency."""
        if self._is_initialized:
            try:
                with self._lock:
                ...
                self.logger.log_error(e, "Failed to start buzzer")
    ...
    def stop_buzz(self):
        ...
        if self._is_initialized:
            try:
                ...
                self.logger.log_error(e, "Failed to stop buzzer")
...
class AccessControlGUI:
    ...
    def _test_buzzer(self):
        ...
        self.hardware.start_buzz(2000)
        time.sleep(0.5)
        self.hardware.stop_buzz()
        ...
        try:
            ...
            if card_details:
                if card_details.is_valid:
                    ...
                    self.hardware.start_buzz(2000)
                    self.hardware.control_green_led(True)
                    self.hardware.control_servo(open_gate=True)
                    time.sleep(2)
                    self.hardware.stop_buzz()
                    self.hardware.control_green_led(False)
                else:
                    access_status = AccessStatus.DENIED
                    ...
                    elif not card_details.is_valid:
                        details += " (Inactive)"
                    self.logger.log_warning(details)
                    self.hardware.start_buzz(1000)
                    self.hardware.control_red_led(True)
                    time.sleep(2)
                    self.hardware.stop_buzz()
                    self.hardware.control_red_led(False)
            else:
                access_status = AccessStatus.DENIED
